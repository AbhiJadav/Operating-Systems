 ...
 * 
 */
public class IRR extends MyAlgorithm{
	 ...
	 */
	public IRR(Queue workQueue , int quantum)
	{
		super(workQueue);
	}
	// function to calculate time quantum
	// Calculate average of  median and high burst time
	public void calculateQuantum() {
		...
		int x = q_copy.size();
		int max = q_copy.getJob(0).remaining;
		if (x >1){
		for(int j =1;j<x;j++){
			if (max < q_copy.getJob(j).remaining){
				max = q_copy.getJob(j).remaining;
			}
		}
		}
		if (x == 1) {
			new_median = q_copy.getJob(0).remaining;
		}
		else{    
			int mid = x/2;
			if (x%2 == 0){
